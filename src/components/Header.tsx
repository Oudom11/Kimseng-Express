...
const Header = () => {
  ...
  const handleLogout = async () => {
    try {
      ...
      await axios.post('https://su24.34.juicyjisu.us/api/logout', {}, {
    ...
    <header className="bg-gray-800 fixed w-full top-0 z-50 shadow-md rounded-b-lg border-b border-gray-700">
      <div className="container mx-auto px-4 py-4 flex justify-between items-center">
        ...
        <nav className="hidden md:flex space-x-8">
          <Link to="/" className={`text-white hover:text-teal-300 transition-colors ${isActive('/') ? 'text-teal-300 font-bold' : ''}`}>{t('home')}</Link>
          <Link to="/routes" className={`text-white hover:text-teal-300 transition-colors ${isActive('/routes') ? 'text-teal-300 font-bold' : ''}`}>{t('routes')}</Link>
          <Link to="/booking" className={`text-white hover:text-teal-300 transition-colors ${isActive('/booking') ? 'text-teal-300 font-bold' : ''}`}>{t('booking')}</Link>
          <Link to="/about" className={`text-white hover:text-teal-300 transition-colors ${isActive('/about') ? 'text-teal-300 font-bold' : ''}`}>{t('about')}</Link>
          <Link to="/contact" className={`text-white hover:text-teal-300 transition-colors ${isActive('/contact') ? 'text-teal-300 font-bold' : ''}`}>{t('contact')}</Link>
        </nav>
        ...
        <div className="hidden md:flex items-center space-x-4">
          ...
          {isLoggedIn ? (
            <Button onClick={handleLogout} className="bg-teal-500 hover:bg-teal-600 rounded-md">
              {t('logout')}
            </Button>
          ) : (
            <Link to="/login">
        ...
        <div className="md:hidden bg-gray-800 border-t rounded-b-lg shadow-lg">
          <nav className="px-4 py-4 space-y-4">
            ...
            <div className="pt-4 space-y-2">
              ...
              {isLoggedIn ? (
                <Button onClick={handleLogout} className="w-full bg-teal-500 hover:bg-teal-600 rounded-md">{t('logout')}</Button>
              ) : (
                <Link to="/login">
